...
if opt.stage2 == 1 then
    net_gen_stage2 = torch.load(opt.checkpoint_dir .. '/' .. opt.net_gen_stage2)
end
...
for query_str in io.lines(opt.queries) do
  ...
  if opt.stage2 == 0 then
    real_img = data:getImages()
  else
...
for i = 1, #fea_txt do
  ...
  local images = net_gen:forward{cur_fea_img:cuda(), cur_fea_txt:cuda()}
  if opt.stage2 == 1 then
    images_128 = net_gen_stage2:forward({{cur_fea_img_128:cuda(), images}, cur_fea_txt:cuda()})
  end
  ...
  images_show:narrow(1, 9, 8):copy(images:narrow(1, 1, 8))
  images_show:narrow(1, 17, 8):copy(ori_img[i]:narrow(1, 9, 8))
  images_show:narrow(1, 25, 8):copy(images:narrow(1, 9, 8))
  images_show:narrow(1, 33, 8):copy(ori_img[i]:narrow(1, 17, 8))
  images_show:narrow(1, 41, 8):copy(images:narrow(1, 17, 8))

  if opt.stage2 == 1 then
    images_show_128:narrow(1, 1, 8):copy(ori_img_128[i]:narrow(1, 1, 8))
    images_show_128:narrow(1, 9, 8):copy(images_128:narrow(1, 1, 8))
    images_show_128:narrow(1, 17, 8):copy(ori_img_128[i]:narrow(1, 9, 8))
    images_show_128:narrow(1, 25, 8):copy(images_128:narrow(1, 9, 8))
    images_show_128:narrow(1, 33, 8):copy(ori_img_128[i]:narrow(1, 17, 8))
    images_show_128:narrow(1, 41, 8):copy(images_128:narrow(1, 17, 8))

    image.save(fname_png_128, image.toDisplayTensor(images_show_128,4,opt.batchSize/3))
  end

  --[[
  image.save('./testx1.png', images:narrow(1, 1, 8)[1]:mul(0.5))
  image.save('./testx2.png', images:narrow(1, 1, 8)[2]:mul(0.5))
  image.save('./testx3.png', images:narrow(1, 1, 8)[3]:mul(0.5))
  image.save('./testx4.png', images:narrow(1, 1, 8)[4]:mul(0.5))
  --]]
